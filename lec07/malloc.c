...
void *my_malloc(size_t size) {
  ...
  while (metadata)
    {
      if (metadata->size == size)
      {
        ...
        temp_prev = prev;
        break;
      } else if (metadata->size > size && (temp_metadata == NULL ||diff_size > metadata->size - size)) {
      diff_size = metadata->size - size;
      temp_metadata = metadata;
      temp_prev = prev;
    }
    prev = metadata;
  ...
  if (!metadata) {
    ...
    //     <---------------------->
    //            buffer_size
    size_t buffer_size = size + sizeof(my_metadata_t);
